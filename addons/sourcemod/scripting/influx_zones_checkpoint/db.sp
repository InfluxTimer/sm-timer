...
stock void DB_InitClientCPTimes( int client )
{
    ...
    FormatEx( szQuery, sizeof( szQuery ),
        ...
        "cpnum," ...
        "cptime " ...
        "FROM "...INF_TABLE_CPTIMES..." AS _cp WHERE uid=%i AND mapid=%i AND " ...
        
        "cptime=(SELECT " ...
            "MIN(cptime) " ...
            "FROM "...INF_TABLE_CPTIMES..." WHERE uid=_cp.uid AND mapid=_cp.mapid AND runid=_cp.runid AND mode=_cp.mode AND style=_cp.style AND cpnum=_cp.cpnum" ...
        ") " ...
...
stock void DB_InitCPTimes( int runid = -1, int mode = -1, int style = -1, int cpnum = -1 )
{
    ...
    FormatEx( szQuery, sizeof( szQuery ),
        ...
        "cpnum," ...
        "cptime " ...
        "FROM "...INF_TABLE_TIMES..." AS _t INNER JOIN "...INF_TABLE_CPTIMES..." AS _cp ON _t.uid=_cp.uid AND _t.mapid=_cp.mapid AND _t.runid=_cp.runid AND _t.mode=_cp.mode AND _t.style=_cp.style WHERE _t.mapid=%i%s " ...
        ...
        "GROUP BY _t.uid,_t.runid,_t.mode,_t.style,cpnum " ...
    ...
    FormatEx( szQuery, sizeof( szQuery ), "SELECT " ...
        ...
        // Requires uid here if ONLY_FULL_GROUP_BY is enabled. Default enabled starting MySQL 5.7.5
        // This shouldn't change behaviour.
        "GROUP BY uid,runid,mode,style,cpnum " ...
        "ORDER BY runid,cpnum",
        mapid,
        szWhere );
